...
export default function PasswordReset({ session }) {
  ...
  useEffect(() => {
    document.title = `Passwort ${session ? "ändern" : isMigrating ? "erstellen" : "zurücksetzen"} | DANAMEME`;
  }, [isMigrating, session]);
  ...
  if (success) {
    return (
      <>
        <h1 className="title">Passwort {session ? "ändern" : isMigrating ? "erstellen" : "zurücksetzen"}</h1>
        <div className="flex flex-col w-full mt-5">
          <p className="text mb-4">
            Dein Passwort wurde erfolgreich {session ? "geändert" : isMigrating ? "erstellt" : "zurückgesetzt"}!
          </p>
          {session ? (
