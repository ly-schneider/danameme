    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-row justify-between items-center">
        ...
        <div className="flex flex-row items-center gap-3">
          ...
          {session.user.id === post.accountId && (
            <DropdownMenu>
              <DropdownMenuTrigger>
      ...
      <div className="flex flex-row items-center gap-3">
        ...
        <Link href={`/post/${post._id}`} className="ms-1.5 text flex flex-row items-center gap-2"><FontAwesomeIcon icon={faComment} className="text-2xl" />0</Link>
      </div>
    </div>
  )
}